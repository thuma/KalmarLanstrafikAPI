...
if(is_file($namelist) == FALSE)
	...
	for($i = 0; $i < count($bokstaver); $i++)
		...
		while($newdata)
			...
			$newdata = FALSE;
			$doc = new DOMDocument();
			@$doc->loadHTML(file_get_contents('http://193.45.213.123/kalmar/indexes.aspx?optType=0&selKommun=0&sLetter='.urlencode(utf8_decode($bokstaver[$i])).'&iPage='.$page.'&Language=se&optFrTo=0&TNSource=KALMAR'));
...
if(is_file($idlist) == FALSE)
	...
	foreach($all as $key => $name)
		{
		$url = 'http://193.45.213.123/kalmar/rpajax.aspx?net=KALMAR&lang=se&letters='.urlencode($name->cleanname);
...
foreach($all as $key => $station)
	...
	if(isset($station->position)==FALSE)
		{
		//open connection
		$ch = curl_init('http://193.45.213.123/kalmar/querypage_adv.aspx');
		...
		.'&selDirection=0'
		.'&inpTime=08%3A51&inpDate=2013-10-22'
		.'&optReturn=0&selDirection2=0'
		.'&inpTime2=12%3A53&inpDate2=2013-10-22'
		.'&trafficmask=1'
